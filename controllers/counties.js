...
exports.selectCounties = async (req, res) => {
    const text = "SELECT county_name, cbsa, fips, COALESCE(pop_score,0) + COALESCE(pop_grow_score,0)+ COALESCE(emp_score,0)+ COALESCE(grp_score,0)+ COALESCE(grp_grow_score,0)+ COALESCE(sw_score,0)+ COALESCE(unemp_score,0) AS total, pop_score, pop_grow_score, emp_score, grp_score, grp_grow_score, sw_score, unemp_score FROM county_score WHERE cbsa = $1";
